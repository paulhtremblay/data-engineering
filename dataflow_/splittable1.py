...
absolute_path_to_dummy_data_file = 'input/file1.txt'
...
class FileToWordsRestrictionProvider(beam.transforms.core.RestrictionProvider
    ...
    def initial_restriction(self, file_name):
        print(OffsetRange(0, os.stat(file_name).st_size))
...
class FileToWordsFn(beam.DoFn):
    def process(
        ...
        with open(file_name) as file_handle:
            ...
            print(f'restriction is {tracker.current_restriction()}')
            while tracker.try_claim(file_handle.tell()):
                print(f'tell is {file_handle.tell()}')
